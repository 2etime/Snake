...
class Mouse {
    ...
    public static func GetMouseViewportPosition()->float2{
        let x = (overallMousePosition.x - GameView.ScreenSize.x * 0.5) / (GameView.ScreenSize.x * 0.5)
        let y = (overallMousePosition.y - GameView.ScreenSize.y * 0.5) / (GameView.ScreenSize.y * 0.5)
        return float2(x, y)
    }
}
